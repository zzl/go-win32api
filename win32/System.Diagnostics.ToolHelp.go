...
func Heap32First(lphe *HEAPENTRY32, th32ProcessID uint32, th32HeapID uintptr) (BOOL, WIN32_ERROR) {
	...
	ret, _,  err := syscall.SyscallN(addr, uintptr(unsafe.Pointer(lphe)), uintptr(th32ProcessID), uintptr(th32HeapID))
	return BOOL(ret), WIN32_ERROR(err)
}
...
func Toolhelp32ReadProcessMemory(th32ProcessID uint32, lpBaseAddress unsafe.Pointer, lpBuffer unsafe.Pointer, cbRead uintptr, lpNumberOfBytesRead *uintptr) BOOL {
	...
	ret, _,  _ := syscall.SyscallN(addr, uintptr(th32ProcessID), uintptr(lpBaseAddress), uintptr(lpBuffer), uintptr(cbRead), uintptr(unsafe.Pointer(lpNumberOfBytesRead)))
	return BOOL(ret)
}
